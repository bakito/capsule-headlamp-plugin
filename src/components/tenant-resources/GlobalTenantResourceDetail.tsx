...
export function GlobalTenantResourceDetail(props: GlobalTenantResourceDetailProps) {
  const params = useParams<{ name: string }>();
  const { name = params.name } = props;

  const [list, listError] = GlobalTenantResource.useList();
  const listedTenantResourceItem = useMemo(
    () => list?.find((q: any) => q.getName() === name),
    [list, name]
  );
  ...
  useEffect(() => {
    const handleRefresh = (event: Event) => {
      ...
      if (detail?.kind === 'GlobalTenantResource' && detail.name === name) {
        setActionRefreshedItem(new GlobalTenantResource(detail.data));
      }
    };
    ...
    <>
      {listError && (
        <Typography color="error" sx={{ mb: 2 }}>
          Error loading GlobalTenantResource: {listError.message || String(listError)}. Is the
      ...
      <Resource.DetailsGrid
        name={name}
        resourceType={GlobalTenantResource}
        extraInfo={item => {
          if (!item) return [];
      ...
      >
        <DetailsSectionStack>
          ...
          <GlobalTenantResourceDefinedResources name={name} item={tenantResourceItem} />
          <ManagedResources
...
function GlobalTenantResourceDefinedResources({
  name,
  item: providedItem,
}: {
  name: string;
  ...
  const [list] = GlobalTenantResource.useList();
  const item = providedItem || list?.find((q: any) => q.getName() === name);
