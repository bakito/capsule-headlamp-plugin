...
import {
  ...
  type BreakRequestTemplateResource,
...
export interface GlobalBreakRequestTemplateDetailProps {
  name?: string;
}

export interface BreakRequestTemplateDetailProps {
  name?: string;
  namespace?: string;
}
...
function BreakRequestTemplateDetailView({
  namespaced,
  requestedName,
  requestedNamespace,
}: {
  ...
  const resourceType: any = namespaced ? BreakRequestTemplate : GlobalBreakRequestTemplate;
    ...
    return (
      <SectionBox title={`Template: ${name}`}>
        <Alert severity="error">
          {namespaced ? 'BreakRequestTemplate' : 'GlobalBreakRequestTemplate'} {name} is unavailable
          or this account cannot read it directly.
        </Alert>
      </SectionBox>
    );
  }
      ...
      <DetailsSectionStack>
        ...
        <SectionBox title="Resource Templates">
          {resources.length === 0 ? (
            ...
            <Stack spacing={2}>
              {resources.map((resource: BreakRequestTemplateResource, resourceIndex: number) => (
...
export function GlobalBreakRequestTemplateDetail(props: GlobalBreakRequestTemplateDetailProps) {
  return <BreakRequestTemplateDetailView namespaced={false} requestedName={props.name} />;
}

export function BreakRequestTemplateDetail(props: BreakRequestTemplateDetailProps) {
  return (
    <BreakRequestTemplateDetailView
      namespaced
...
export default GlobalBreakRequestTemplateDetail;
