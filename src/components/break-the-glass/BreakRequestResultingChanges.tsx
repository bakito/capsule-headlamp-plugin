...
import {
  breakRequestTargetDiff,
  breakRequestTargetDiscoveryURL,
  breakRequestTargetURL,
  isKubernetesNotFound,
} from './breakRequestDiff';
...
export function BreakRequestResultingChanges({
  cluster,
  targets,
}: {
  cluster?: string;
  targets: RenderedBreakRequestTarget[];
}) {
  ...
  useEffect(() => {
    ...
    const discoverResourceName = async (target: RenderedBreakRequestTarget) => {
      const discoveryURL = breakRequestTargetDiscoveryURL(target);
    ...
    for (const target of targets) {
      discoverResourceName(target)
        .then(resourceName =>
          ApiProxy.request(breakRequestTargetURL(target, resourceName), { cluster })
        )
        .then(live => {
          if (!active) return;
    ...
    <Stack gap={2}>
      ...
      {targets.map(target => {
        ...
        const changes = state.error
          ? []
          : breakRequestTargetDiff(state.missing ? undefined : state.live, target.manifest);
...
export default BreakRequestResultingChanges;
