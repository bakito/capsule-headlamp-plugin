...
import { templateApprovalMode, templateNamespaces } from './breakRequestHelpers';
import {
...
import { BreakRequestTemplateIdentity } from './BreakRequestTemplatePresentation';
import { BreakRequestYamlDialog } from './BreakRequestYamlDialog';
...
type BreakRequestTemplateKind = 'BreakRequestTemplate' | 'GlobalBreakRequestTemplate';

interface SelectableBreakRequestTemplate {
  key: string;
  kind: BreakRequestTemplateKind;
  namespace?: string;
  template: any;
}
...
export function openCreateBreakRequestActivity(namespaces: string[] = []) {
  const cluster = currentCluster();
  Activity.launch({
    id: `capsule-create-break-request ${[...namespaces].sort().join(',')} ${cluster || ''}`,
    title: 'New BreakRequest',
    hideTitleInHeader: true,
    location: 'split-right',
    temporary: true,
    cluster,
    content: <CreateBreakRequest cluster={cluster} namespaces={namespaces} />,
    icon: <Icon icon="mdi:shield-plus-outline" width="100%" height="100%" />,
  });
}

export function NewBreakRequestButton({ namespaces }: { namespaces: string[] }) {
  return (
    <Button
      onClick={() => openCreateBreakRequestActivity(namespaces)}
      size="large"
...
function BreakRequestSetupForm({
  cluster,
  namespaces,
  templateKind,
  templateName,
  ...
  cluster?: string;
  namespaces: string[];
  templateKind: BreakRequestTemplateKind;
  templateName: string;
  templateNamespace?: string;
}) {
  const resourceType: any =
    templateKind === 'BreakRequestTemplate' ? BreakRequestTemplate : GlobalBreakRequestTemplate;
  const [template, templateError] = resourceType.useGet(
    templateName,
    templateKind === 'BreakRequestTemplate' ? templateNamespace : undefined,
    { cluster }
  );
  ...
  const uiSchema = useMemo(() => buildBreakRequestUiSchema(paramSchema), [paramSchema]);
  const initialValues = useMemo(() => breakRequestDefaultParamData(paramSchema), [paramSchema]);
  ...
  const currentRequest = (params: any) => {
    ...
    return buildCreateBreakRequest({
      duration,
  ...
  const submit = async (params: any) => {
    ...
    try {
      ...
      const createdName =
        response?.metadata?.name || response?.data?.metadata?.name || request.name;
      setCreated({ name: createdName, namespace: request.namespace });
      enqueueSnackbar(`Created BreakRequest ${request.namespace}/${createdName}`, {
        variant: 'success',
      });
    } catch (error: any) {
      const message =
      ...
      enqueueSnackbar(`Failed to create BreakRequest: ${message}`, { variant: 'error' });
    } finally {
      setSubmitting(false);
    }
  };
      ...
      {activeStep === 0 ? (
        <SectionBox title="Setup">
          <Stack gap={3}>
            ...
            >
              <Stack gap={2}>
                <BreakRequestTemplateIdentity template={template} />
            ...
            <Paper variant="outlined" sx={{ borderWidth: 2, p: { xs: 2, sm: 3 } }}>
              <Stack gap={2.75}>
                ...
                >
                  <Stack gap={1.5}>
                    ...
                    <TextField
                      ...
                      label={generateName ? 'BreakRequest name prefix' : 'BreakRequest name'}
                ...
                <TextField
                  ...
                  fullWidth
                  helperText={errors.reason || 'Recorded in the BreakRequest audit trail.'}
                  label="Reason"
                  minRows={4}
                  multiline
        ...
        <SectionBox title="Template Parameters">
          <Stack gap={2.5}>
            ...
            >
              <BreakRequestTemplateIdentity compact template={template} />
            </Paper>
            <Form
...
export function CreateBreakRequest({
  cluster,
  namespaces,
}: {
  cluster?: string;
  ...
  const [globalTemplates, globalError] = GlobalBreakRequestTemplate.useList({ cluster });
  const [namespacedTemplates, namespacedError] = BreakRequestTemplate.useList({
    cluster,
  ...
  const availableTemplates = useMemo(
    (): SelectableBreakRequestTemplate[] =>
      [
        ...(namespacedTemplates || [])
          .filter(
            template =>
          ...
          .map(template => ({
            ...
            kind: 'BreakRequestTemplate' as const,
        ...
        ...(globalTemplates || [])
          ...
          .map(template => ({
            key: templateCatalogKey('GlobalBreakRequestTemplate', undefined, template.getName()),
            kind: 'GlobalBreakRequestTemplate' as const,
            namespace: undefined,
            template,
          })),
      ].sort(
  ...
  const filteredTemplates = useMemo(
    () => filterBreakRequestTemplates(availableTemplates, search, selectedTags),
    [availableTemplates, search, selectedTags]
  );
  const templateGroups = useMemo(
    () => groupBreakRequestTemplates(filteredTemplates),
    [filteredTemplates]
  );
  ...
  const unavailable = [
    globalError ? 'GlobalBreakRequestTemplate' : '',
    namespacedError ? 'BreakRequestTemplate' : '',
  ].filter(Boolean);

  return (
    <>
      <SectionBox title="New BreakRequest">
        <Paper
          variant="outlined"
          sx={theme => ({
        ...
        >
          ...
          {globalError && namespacedError ? (
            <Alert severity="error">
              ...
              BreakRequestTemplates in the selected Namespace scope or GlobalBreakRequestTemplates.
            </Alert>
          ) : loading ? (
                ...
                <Stack gap={3}>
                  {templateGroups.map(group => (
                    <Box key={group.category}>
                      ...
                      >
                        {group.templates.map((entry: SelectableBreakRequestTemplate) => {
                                ...
                                <CardContent sx={{ height: '100%' }}>
                                  <Stack gap={1.5} height="100%">
                                    ...
                                    >
                                      ...
                                      <Chip
                                        color={
                                          kind === 'BreakRequestTemplate' ? 'secondary' : 'info'
                                        }
                                        label={
